...
void MPI_1D_blocking<real_t>::prepare(const max_problem_t& problem)
{
    ...
    if (static_cast<index_t>(problem.nz % mpi_size) > mpi_rank) // Distribute residuals when division is not perfect
        local_z_nodes++;
    ...
    z_ini = init_z_node;
    z_end = init_z_node + local_z_nodes;
    y_ini = 0;
    y_end = problem.ny;
    x_ini = 0;
    x_end = problem.nx;
...
double MPI_1D_blocking<real_t>::access(std::size_t s, std::size_t x, std::size_t y, std::size_t z) const
{
    ...
    int root_rank = -1;
    /*
    if (z >= init_z_node && z < (init_z_node + local_z_nodes)) {
        //value = static_cast<real_t>(noarr::get_at<'s', 'x', 'y', 'z'>(substrates_.get(), s, x, y, z - init_z_node));
        value = substrates_[(z - init_z_node)*thomas_k_jump + y * thomas_j_jump + x * thomas_i_jump + s];
        root_rank = mpi_rank;
    }
    
    else{
        index_t base_z_nodes = problem_.nz / mpi_size;
        index_t remainder = problem_.nz % mpi_size;
         // Compute root rank
        if (z < (remainder * (base_z_nodes + 1))) {
            root_rank =  z / (base_z_nodes + 1);  // First 'remainder' ranks get extra node
        } else {
            root_rank = (z - remainder) / base_z_nodes;  // Remaining ranks
        }
    }
    // Broadcast the value from the owner process to all others
    if (root_rank < 0 or root_rank >= mpi_size) {
        std::cout << "Error at rank " << mpi_rank << " pos: " << z << " " << y << " " << x << " " << s << " root rank: " << root_rank << std::endl;
    }
    if (mpi_rank == root_rank) {
        // El proceso dueño envía el valor a todos los demás procesos
        for (int dest = 0; dest < mpi_size; ++dest) {
            if (dest != root_rank) {
                MPI_Send(&value, 1, MPI_DOUBLE, dest, 0, mpi_comm);
            }
        }
    } else {
        // Todos los demás procesos reciben el valor
        MPI_Recv(&value, 1, MPI_DOUBLE, root_rank, 0, mpi_comm, MPI_STATUS_IGNORE);
    }
    MPI_Barrier(mpi_comm);*/

    value = substrates_[z*thomas_k_jump + y * thomas_j_jump + x * thomas_i_jump + s];
	return value;
}
