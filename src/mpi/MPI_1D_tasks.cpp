...
void MPI_1D_tasks<real_t>::precompute_values()
{
...
template <typename real_t>
void MPI_1D_tasks<real_t>::precompute_values_vec( std::int32_t vl)
{
...
*/
template <typename real_t>
void MPI_1D_tasks<real_t>::initialize()
{
	precompute_values();
}
template <typename real_t>
auto MPI_1D_tasks<real_t>::get_substrates_layout(const problem_t<index_t, real_t>& problem, const index_t x_nodes, const index_t y_nodes, const index_t z_nodes)
{
...
template <typename real_t>
void MPI_1D_tasks<real_t>::prepare(const max_problem_t& problem)
{
...
template <typename real_t>
void MPI_1D_tasks<real_t>::solve()
{
	solve_x();
	solve_y();
	solve_z();
...
template <typename real_t>
void MPI_1D_tasks<real_t>::solve_x()
{
...
template <typename real_t>
void MPI_1D_tasks<real_t>::solve_y()
{
	#pragma omp parallel for collapse(2)
	for (index_t k = 0; k < local_z_nodes; k++)
	{
...
template <typename real_t>
void MPI_1D_tasks<real_t>::solve_z()
{
...
template <typename real_t>
void MPI_1D_tasks<real_t>::save(std::ostream& out) const
{
...
template <typename real_t>
double MPI_1D_tasks<real_t>::access(std::size_t s, std::size_t x, std::size_t y, std::size_t z) const
{
    //std::cout << "###############################################" <<  std::endl;
	auto dens_l = get_substrates_layout(problem_, local_x_nodes, local_y_nodes, local_z_nodes);
...
template <typename real_t>
void MPI_1D_tasks<real_t>::tune(const nlohmann::json& params)
{
...
template class MPI_1D_tasks<float>;
template class MPI_1D_tasks<double>;
