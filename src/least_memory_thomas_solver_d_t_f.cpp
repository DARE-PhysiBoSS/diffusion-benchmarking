#include "least_memory_thomas_solver_d_t_f.h"
...
void least_memory_thomas_solver_d_t_f<real_t, aligned_x>::precompute_values(real_t*& a, real_t*& b1, real_t*& c,
...
template <typename real_t, bool aligned_x>
void least_memory_thomas_solver_d_t_f<real_t, aligned_x>::prepare(const max_problem_t& problem)
{
...
template <typename real_t, bool aligned_x>
void least_memory_thomas_solver_d_t_f<real_t, aligned_x>::tune(const nlohmann::json& params)
{
...
template <typename real_t, bool aligned_x>
void least_memory_thomas_solver_d_t_f<real_t, aligned_x>::initialize()
{
...
template <typename real_t, bool aligned_x>
auto least_memory_thomas_solver_d_t_f<real_t, aligned_x>::get_diagonal_layout(const problem_t<index_t, real_t>& problem,
																			  index_t n)
{
...
template <typename index_t, typename real_t, typename density_layout_t, typename diagonal_layout_t>
static void solve_slice_xy_fused_transpose(real_t* __restrict__ densities, const real_t* __restrict__ a,
										   const real_t* __restrict__ b1, const real_t* __restrict__ back_c,
										   const density_layout_t dens_l, const diagonal_layout_t diag_l,
										   const index_t s_begin, const index_t s_end)
{
	...
	for (index_t s = s_begin; s < s_end; s++)
	{
		for (index_t z = 0; z < z_len; z++)
		{
			...
			// vectorized body
			{
				...
				for (index_t y = 0; y < y_len; y++)
				{
					...
					for (index_t i = 0; i < full_n - simd_length; i += simd_length)
					{
						...
						for (index_t v = 0; v < simd_length; v++)
							rows[v + 1] = hn::Load(
								d, &(body_dens_l | noarr::get_at<'z', 'y', 'v', 'x', 's'>(densities, z, y, v, i, s)));
					...
					// here we fuse the end of forward substitution and the beginning of backwards propagation
					{
						...
						// aligned loads
						for (index_t v = 0; v < simd_length; v++)
							rows[v + 1] = hn::Load(d, &(body_dens_l
			...
			// yz remainder
			{
				...
				for (index_t y = 0; y < v_len; y++)
				{
					const real_t a_s = a[s];
					const real_t b1_s = b1[s];
					const auto d =
						noarr::make_bag(rem_dens_l ^ noarr::fix<'s', 'z', 'y', 'v'>(s, z, noarr::lit<0>, y), densities);
					const auto c = noarr::make_bag(diag_l ^ noarr::fix<'s'>(s), back_c);
...
template <typename real_t, bool aligned_x>
void least_memory_thomas_solver_d_t_f<real_t, aligned_x>::solve_x()
{
	if (use_intrinsics_)
	{
		if (this->problem_.dims == 1)
...
#pragma omp parallel
		...
		{
#pragma omp parallel
			solve_slice_xy_fused_transpose<index_t>(this->substrates_, ax_, b1x_, cx_, get_substrates_layout<3>(),
													get_diagonal_layout(this->problem_, this->problem_.nx), 0,
													this->problem_.substrates_count);
		}
...
template <typename real_t, bool aligned_x>
void least_memory_thomas_solver_d_t_f<real_t, aligned_x>::solve_y()
{
	if (use_intrinsics_)
	{
#pragma omp parallel
...
template <typename real_t, bool aligned_x>
void least_memory_thomas_solver_d_t_f<real_t, aligned_x>::solve_z()
{
	if (use_intrinsics_)
	{
#pragma omp parallel
...
template <typename real_t, bool aligned_x>
void least_memory_thomas_solver_d_t_f<real_t, aligned_x>::solve()
{
	if (this->problem_.dims == 1)
	{
		solve_1d();
...
template <typename real_t, bool aligned_x>
void least_memory_thomas_solver_d_t_f<real_t, aligned_x>::solve_1d()
{
	if (!use_intrinsics_)
	{
#pragma omp parallel
...
template <typename real_t, bool aligned_x>
void least_memory_thomas_solver_d_t_f<real_t, aligned_x>::solve_2d()
{
	// 	if (!use_intrinsics_)
	// 	{
	// #pragma omp parallel
...
template <typename real_t, bool aligned_x>
void least_memory_thomas_solver_d_t_f<real_t, aligned_x>::solve_3d()
{
	if (!use_intrinsics_)
	{
#pragma omp parallel
...
template <typename real_t, bool aligned_x>
least_memory_thomas_solver_d_t_f<real_t, aligned_x>::least_memory_thomas_solver_d_t_f(bool use_intrinsics)
	: ax_(nullptr),
	  b1x_(nullptr),
	  cx_(nullptr),
	  ay_(nullptr),
...
template <typename real_t, bool aligned_x>
least_memory_thomas_solver_d_t_f<real_t, aligned_x>::~least_memory_thomas_solver_d_t_f()
{
	if (cx_)
	{
		std::free(cx_);
...
template class least_memory_thomas_solver_d_t_f<float, false>;
template class least_memory_thomas_solver_d_t_f<double, false>;

template class least_memory_thomas_solver_d_t_f<float, true>;
template class least_memory_thomas_solver_d_t_f<double, true>;
