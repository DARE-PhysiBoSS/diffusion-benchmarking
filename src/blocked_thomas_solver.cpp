...
{
	block_size_ = params.contains("block_size") ? (std::size_t)params["block_size"] : 10;
...
void blocked_thomas_solver<real_t, aligned_x>::initialize()
{
	...
	auto block_size = (max_n + block_size_ - 1) / block_size_;
...
static void solve_slice_x_1d(real_t* __restrict__ densities, const real_t* __restrict__ ac,
							 ...
							 real_t* __restrict__ c, const density_layout_t dens_l, index_t block_size)
{
	const index_t substrates_count = dens_l | noarr::get_length<'s'>();
	const index_t n = dens_l | noarr::get_length<'x'>();

	index_t blocks_count = n / block_size;
	...
	for (index_t s = 0; s < substrates_count; s++)
	{
		...
		{
			index_t block_start = block_idx * block_size;
			index_t block_end = std::min(block_start + block_size, n);
...
void blocked_thomas_solver<real_t, aligned_x>::solve_x()
{
	if (this->problem_.dims == 1)
	{
		...
		solve_slice_x_1d<index_t>(this->substrates_, ax_, b1x_, a_scratch_[get_thread_num()],
								  ...
								  get_substrates_layout<1>(), block_size_);
	}
