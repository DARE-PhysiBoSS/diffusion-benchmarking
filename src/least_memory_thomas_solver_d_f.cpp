...
static void y_blocked_middle(density_bag_t d, const index_t z, scratch_bag_t a, scratch_bag_t c,
							 const index_t coop_size, const index_t s)
{
	const index_t x_len = d | noarr::get_length<'x'>();
	const index_t y_len = d | noarr::get_length<'y'>();
	...
	for (index_t equation_idx = coop_size * 2 - 2; equation_idx >= 0; equation_idx--)
	{
		...
		for (index_t x = 0; x < x_len; x++)
		{
			d.template at<'s', 'x', 'y', 'z'>(s, x, i, z) =
				d.template at<'s', 'x', 'y', 'z'>(s, x, i, z)
				- c[state] * d.template at<'s', 'x', 'y', 'z'>(s, x, next_i, z);
		}
	}
}
...
static void y_blocked_end_with_z(density_bag_t d, const index_t z, scratch_bag_t a, scratch_bag_t c,
								 ...
								 const real_t b1z_s, z_data_t z_data)
{
	...
	for (index_t i = y_begin + 1; i < y_end - 1; i++)
	{
		...
		for (index_t x = 0; x < x_len; x++)
		{
			...
			z_forward_inside_y_blocked(d, s, z, i, x, d.template at<'s', 'x', 'y', 'z'>(s, x, i, z), az_s, b1z_s,
									   z_data.begin, z_data.c);
		}
	}
}
...
template <bool y_blocked, typename index_t, typename real_t, typename density_bag_t, typename diag_bag_t,
		  typename y_data_t>
	...
	for (index_t s = s_begin; s < s_end; s++)
	{
		...
		for (index_t z = 0; z < z_len; z++)
		{
			...
			for (index_t y = 0; y < y_len; y++)
			{
				...
				x_backward<false>(d, cx, s, z, y, prev, ay_s, b1y_s, inside_data { c_tmp_y });
...
template <bool z_blocked, typename vec_t, typename index_t, typename real_t, typename simd_tag,
...
template <bool y_blocked, typename simd_t, typename simd_tag, typename index_t, typename real_t, typename density_bag_t,
	...
	for (index_t s = s_begin; s < s_end; s++)
	{
		...
		for (index_t z = 0; z < z_len; z++)
		{
...

			{
				...
				xy_fused_transpose_part<false, simd_t>(d, t, simd_length, 0, y_len_body, z, s, ax_s, b1x_s, ay_s, b1y_s,
													   cx, inside_data { cy_tmp });
			}
			...
			// y remainder
			{
				...
				for (index_t y = y_len - y_len_remainder; y < y_len; y++)
				{
					...
					x_backward<false>(d, cy, s, z, y, prev, ay_s, b1y_s, inside_data<real_t> { cy_tmp });

					y_forward_inside_x<true>(d, s, z, y, 0, prev, ay_s, b1y_s, cy_tmp);
				}
			}

			y_backward_vectorized<false, simd_t>(dens_l, densities, cy, t, z, s, simd_length, y_len, az_s, b1z_s,
												 inner_z_data { cz_tmp });
		}
	...
	const index_t z_begin, const index_t z_end, const index_t z_block_size, index_t& epoch, std::atomic<long>& counter)
{
	...
	for (index_t z = z_begin; z < z_end; z++)
	{
...

		{
			...
			xy_fused_transpose_part<false, simd_t>(d, t, simd_length, 0, y_len_body, z, s, ax_s, b1x_s, ay_s, b1y_s, cx,
												   inside_data { cy_tmp });
		}
		...
		// y remainder
		{
			...
			for (index_t y = y_len - y_len_remainder; y < y_len; y++)
			{
				...
				x_backward<false>(d, cy, s, z, y, prev, ay_s, b1y_s, inside_data { cy_tmp });

				y_forward_inside_x<true>(d, s, z, y, 0, prev, ay_s, b1y_s, cy_tmp);
			}
		}

		y_backward_vectorized<true, simd_t>(dens_l, densities, cy, t, z, s, simd_length, y_len, az_s, b1z_s,
...
template <typename index_t, typename real_t, typename density_layout_t, typename diagonal_layout_t,
		  typename scratch_layout_t>
static void solve_slice_xy_fused_transpose_blocked(real_t* __restrict__ densities, const real_t* __restrict__ ax,
												   const real_t* __restrict__ b1x, const real_t* __restrict__ back_cx,
												   const real_t* __restrict__ ay, const real_t* __restrict__ b1y,
												   real_t* __restrict__ a_data, real_t* __restrict__ c_data,
												   const density_layout_t dens_l, const diagonal_layout_t diagx_l,
												   const scratch_layout_t scratch_l, const index_t s_begin,
												   const index_t s_end, const index_t y_begin, const index_t y_end,
												   const index_t coop_size, index_t& epoch, std::atomic<long>& counter)
{
	using simd_tag = hn::ScalableTag<real_t>;
	simd_tag t;
	HWY_LANES_CONSTEXPR index_t simd_length = hn::Lanes(t);
	...
	for (index_t s = s_begin; s < s_end; s++)
	{
...

		{
			...
			xy_fused_transpose_part<true, simd_t>(d, t, simd_length, y_begin, y_end - y_remainder, 0, s, ax_s, b1x_s,
												  ay_s, b1y_s, cx,
		...
		// y remainder
		{
			...
			for (index_t y = y_end - y_remainder; y < y_end; y++)
			{
				...
				x_backward<true>(d, cx, s, 0, y, prev, ay_s, b1y_s, inside_data_blocked { y_begin, c_scratch });
		...
		if (y_begin == 0)
			y_blocked_middle(d, 0, a_scratch, c_scratch, coop_size, s);
...
template <typename index_t, typename real_t, typename density_layout_t, typename diagonal_layout_t,
		  typename scratch_layout_t>
static void solve_slice_xyz_fused_transpose_blocked2(
	real_t* __restrict__ densities, const real_t* __restrict__ ax, const real_t* __restrict__ b1x,
	const real_t* __restrict__ back_cx, const real_t* __restrict__ ay, const real_t* __restrict__ b1y,
	const real_t* __restrict__ back_cy, const real_t* __restrict__ az, const real_t* __restrict__ b1z,
	real_t* __restrict__ ay_data, real_t* __restrict__ cy_data, real_t* __restrict__ az_data,
	real_t* __restrict__ cz_data, const density_layout_t dens_l, const diagonal_layout_t diagx_l,
	const diagonal_layout_t diagy_l, const scratch_layout_t scratch_l, const index_t s, const index_t y_begin,
	const index_t y_end, const index_t y_block_size, const index_t z_begin, const index_t z_end,
	const index_t z_block_size, index_t& epoch_y, std::atomic<long>& counter_y, index_t& epoch_z,
	std::atomic<long>& counter_z)
{
	using simd_tag = hn::ScalableTag<real_t>;
	simd_tag t;
	HWY_LANES_CONSTEXPR index_t simd_length = hn::Lanes(t);
	...
	const auto ay_scratch = noarr::make_bag(scratch_l, ay_data);
	const auto cy_scratch = noarr::make_bag(scratch_l, cy_data);

	const auto az_scratch = noarr::make_bag(scratch_l, az_data);
	const auto cz_scratch = noarr::make_bag(scratch_l, cz_data);
	...
	for (index_t z = z_begin; z < z_end; z++)
	{
...

		{
			...
			xy_fused_transpose_part<true, simd_t>(d, t, simd_length, y_begin, y_end - y_remainder, z, s, ax_s, b1x_s,
												  ay_s, b1y_s, cx,
		...
		// y remainder
		{
			...
			for (index_t y = y_len - y_remainder; y < y_len; y++)
			{
				real_t a_tmp = 0;
				real_t b_tmp = b1x_s + ax_s;
				real_t c_tmp = ax_s;
				...
				x_backward<true>(d, cy, s, z, y, prev, ay_s, b1y_s, inside_data_blocked { y_begin, cy });

				y_forward_inside_x_blocked(d, s, z, y, 0, prev, ay_s, b1y_s, y_begin, cy);

				y_forward_inside_x_blocked_next(s, z, ay_s, b1y_s, y_len, y_begin, ay_scratch, cy_scratch);
			}
		}

		y_backward_blocked(d, z, y_begin, y_end, ay_scratch, cy_scratch, s);

		index_t cooperating_threads = (y_len + y_block_size - 1) / y_block_size;

		{
			auto val = counter_y.fetch_add(1, std::memory_order_acq_rel) + 1;

			while (val < epoch_y * cooperating_threads)
			{
		...
		if (y_begin == 0)
			y_blocked_middle(d, z, ay_scratch, cy_scratch, y_block_size, s);
		...
		y_blocked_end_with_z(d, z, ay_scratch, cy_scratch, y_begin, y_end, s, az_s, b1z_s,
		...
		for (index_t i = y_begin + 1; i < y_end - 1; i++)
		{
			...
			for (index_t x = 0; x < x_len; x++)
			{
				z_forward_inside_y_blocked(d, s, z, i, x, d.template at<'s', 'x', 'y', 'z'>(s, x, y_begin, z), az_s,
										   b1z_s, z_begin, cz_scratch);

				z_forward_inside_y_blocked(d, s, z, i, x, d.template at<'s', 'x', 'y', 'z'>(s, x, y_end - 1, z), az_s,
										   b1z_s, z_begin, cz_scratch);
			}
		}
	...
	z_backward_blocked(d, z_begin, z_end, az_scratch, cz_scratch, s);
...

	{
		...
		while (val < epoch_z * cooperating_threads)
		{
	...
	if (z_begin == 0)
		z_blocked_middle(d, az_scratch, cz_scratch, z_block_size, s);
	...
	z_blocked_end(d, az_scratch, cz_scratch, z_begin, z_end, s);
}
...
template <typename real_t, bool aligned_x>
void least_memory_thomas_solver_d_f<real_t, aligned_x>::solve_blocked()
{
		...
		for (index_t i = 0; i < this->problem_.iterations; i++)
		{
			...
			if (use_intrinsics_)
			{
				solve_slice_xyz_fused_transpose_blocked<index_t>(
					...
					get_diagonal_layout(this->problem_, this->problem_.ny),
					get_scratch_layout(this->problem_.nz, this->problem_.nx * this->problem_.ny) ^ noarr::fix<'l'>(s),
...
// 					  << " group: " << substrate_group << std::endl;
			...
			for (index_t i = 0; i < this->problem_.iterations; i++)
			{
				...
				solve_slice_xy_fused_transpose_blocked<index_t>(
					this->substrates_, ax_, b1x_, cx_, ay_, b1y_, a_scratchy_, c_scratchy_, get_substrates_layout<3>(),
					get_diagonal_layout(this->problem_, this->problem_.nx),
					get_scratch_layout(this->problem_.ny, substrate_groups_) ^ noarr::fix<'l'>(substrate_group), s,
					s + s_step_length, block_y_begin, block_y_end, group_size, epoch,
					countersy_[substrate_group].value);
			}
		}
	}
}
