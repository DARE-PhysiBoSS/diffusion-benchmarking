...
void biofvm_par<real_t>::precompute_values()
{
...
template <typename real_t>
void biofvm_par<real_t>::initialize()
{
	precompute_values();
}
template <typename real_t>
auto biofvm_par<real_t>::get_substrates_layout(const problem_t<index_t, real_t>& problem)
{
...
template <typename real_t>
void biofvm_par<real_t>::prepare(const max_problem_t& problem)
{
...
template <typename real_t>
void biofvm_par<real_t>::solve_x()
{
#pragma omp parallel for collapse(3)
	for (index_t k = 0; k < problem_.nz; k++)
		for (index_t j = 0; j < problem_.ny; j++)
			for (index_t d = 0; d < problem_.substrates_count; d++) {
				long_index_t index = k * thomas_k_jump + j * thomas_j_jump +d;
				substrates_[index] /= bx_[0][d];
				...
				for (index_t i = 1; i < problem_.nx; i++)
				{
					...
					substrates_[index_inc] += constant1[d] * substrates_[index];
					
					//(*(*M.p_density_vectors))[n] /= M.thomas_denomz[k];
					
					substrates_[index_inc] /= bx_[i][d];
					

					index = index_inc;
				}

				index = k * thomas_k_jump + j * thomas_j_jump + (thomas_i_jump * (problem_.nx - 1)) +d;
				for (index_t i = problem_.nx - 2; i >= 0; i--)
				{
					...
					substrates_[index_dec] -= cx_[i][d] * substrates_[index];
...
template <typename real_t>
void biofvm_par<real_t>::solve_y()
{
#pragma omp parallel for collapse(3)
	for (index_t k = 0; k < problem_.nz; k++)
		for (index_t i = 0; i < problem_.nx; i++)
			for (index_t d = 0; d < problem_.substrates_count; d++) {
				long_index_t index = k * thomas_k_jump + i * thomas_i_jump +d;
				
				substrates_[index] /= by_[0][d];
				...
				for (index_t j = 1; j < problem_.ny; j++)
				{
					...
					substrates_[index_inc] /= by_[j][d];

					index = index_inc;
				}

				index = k * thomas_k_jump + i * thomas_i_jump + (thomas_j_jump * (problem_.ny - 1)) +d;
...
template <typename real_t>
void biofvm_par<real_t>::solve_z()
{
#pragma omp parallel for collapse(3)
	for (index_t j = 0; j < problem_.ny; j++)
		for (index_t i = 0; i < problem_.nx; i++)
			for (index_t d = 0; d < problem_.substrates_count; d++) {
				long_index_t index = j * thomas_j_jump + i * thomas_i_jump +d; 
				...
				index = i * thomas_i_jump + j * thomas_j_jump + (thomas_k_jump * (problem_.nz - 1)) +d;
				for (index_t k = problem_.nz - 2; k >= 0; k--)
				{
					long_index_t index_dec = index - thomas_k_jump;
					// naxpy(&(*(*M.p_density_vectors))[n], M.thomas_cz[k], (*(*M.p_density_vectors))[n + M.thomas_k_jump]);
					
					substrates_[index_dec] -= cz_[k][d] * substrates_[index];
...
template <typename real_t>
void biofvm_par<real_t>::save(std::ostream& out) const
{
...
template <typename real_t>
double biofvm_par<real_t>::access(std::size_t s, std::size_t x, std::size_t y, std::size_t z) const
{
	auto dens_l = get_substrates_layout(problem_);
...
template class biofvm_par<float>;
template class biofvm_par<double>;
