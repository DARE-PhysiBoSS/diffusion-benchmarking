...
#ifdef USE_MPI
	#include "mpi/MPI_1D_blocking.h"
	#include "mpi/MPI_helper.h"
#endif
...
std::map<std::string, std::function<std::unique_ptr<diffusion_solver>()>> get_solvers_map()
{
	std::map<std::string, std::function<std::unique_ptr<diffusion_solver>()>> solvers;
		...
		solvers.emplace("biofvm", []() { return std::make_unique<reference_thomas_solver<real_t>>(); });
	...
	solvers.emplace("biofvm_par", []() { return std::make_unique<biofvm_par<real_t>>(); });
													  ...
													  const max_problem_t& problem)
{
	...
	#ifdef USE_MPI
	std::size_t z_ini = alg.z_min;
	std::size_t z_end = alg.z_max;
	std::size_t y_ini = alg.y_min;
	std::size_t y_end = alg.y_max;
	std::size_t x_ini = alg.x_min;
	std::size_t x_end = alg.x_max;
	#else
	std::size_t z_ini = 0;
	...
	int rank;
	MPI_Comm_rank(alg.mpi_comm, &rank);
	MPI_Reduce( &maximum_absolute_difference,&rcv_max, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
	MPI_Reduce( &se, &rcv_se, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	
	maximum_absolute_difference = rcv_max;
	se = rcv_se;
	#endif
	double rmse = std::sqrt(se / (problem.nz * problem.ny * problem.nx * problem.substrates_count));
