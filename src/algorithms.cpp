...
std::map<std::string, std::function<std::unique_ptr<diffusion_solver>()>> get_solvers_map()
{
	...
	solvers.emplace("sdd-lstmtai",
					[]() { return std::make_unique<sdd_least_memory_thomas_solver_t<real_t, true>>(false); });
	solvers.emplace("sdd-lstmtaic",
					[]() { return std::make_unique<sdd_least_memory_thomas_solver_t<real_t, true>>(true); });
	solvers.emplace("sdd-pb", []() { return std::make_unique<sdd_partial_blocking<real_t, true>>(false); });
	solvers.emplace("sdd-pbc", []() { return std::make_unique<sdd_partial_blocking<real_t, true>>(true); });
